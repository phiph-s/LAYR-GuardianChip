...
\begin{enumerate}
  ...
  \item \textbf{NFC Interface} (\texttt{mfrc522\_apdu\_interface}) –
        ...
        provides a byte-stream APDU interface to the authentication core.
...
It operates in two main phases, shown in \cref{fig:nfc_state}:
...
been present in an intermediate design, halving the silicon area
required for the cryptographic portion of the chip.
...
\paragraph{Simulator limitation: screen-lock dependency.}
Android's HCE framework only routes NFC APDUs to a registered
\texttt{HostApduService} when the device screen is on and the phone
is unlocked~\cite{android_hce}.
In practice this means the simulator cannot be used as a passive,
always-on card emulator: the phone must be actively unlocked before
each test transaction.
This is not a limitation of the protocol or the FPGA design, but it
is worth noting for anyone reproducing the test setup.
...
The process was selected because it is accessible through the
LibreLane/OpenLane open-source ASIC flow~\cite{librelane,shalan2020openlane}
and was explicitly mandated by the LAYR challenge specification~\cite{ocdc_layr}.
...
\begin{itemize}
  ...
  \item The single-cycle AES implementation was replaced by the custom
        ...
        the AES-CTR construction, reducing the logic footprint
